...
export default function BillingDetail() {
  const { billingId, setBillingId } = useBillingManagemnt();
  const [billingDetail, setBillingDetail] = useState<BillingDetailType | null>(
    null,
  );
  ...
  useEffect(() => {
    async function getBillingDetail() {
      if (!billingId) return;
      const response = await BillingService.getBillingDetail(billingId);
      if (response) {
  ...
  if (!billingDetail) {
    return (
      <div className="bg-east- bay backdrop-blur-md rounded p-12 text-center border border-silver">
        ...
        <p className="text-east-bay">Chọn một hóa đơn để xem chi tiết</p>
      </div>
    );
  }
    ...
    <>
      <div className="bg-white rounded-2xl p-6 md:p-8 border border-gray-200 shadow-lg">
        ...
        <div className="mb-6 pb-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-martinique mb-1">
            Chi Tiết Hóa Đơn
          </h2>
